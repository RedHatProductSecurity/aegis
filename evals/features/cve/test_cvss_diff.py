...
def is_cvss_valid(cvss: str) -> bool:
    return re.match(
        r"([0-9]+(\.[0-9])?)?[/]?(CVSS:3\.1/AV:./AC:./PR:./UI:./S:./C:./I:./A:.)?", cvss
    )
...
class CVSSDiffEvaluator(Evaluator[str, CVSSDiffExplainerModel]):
    async def evaluate(self, ctx) -> EvaluationReason:
        ...
        if not is_cvss_valid(rh_cvss) or not is_cvss_valid(nvd_cvss):
            return make_eval_reason(
                fail_reason="invalid CVSS score returned by the agent"
            )
